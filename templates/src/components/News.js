    ...
    render() { 
        return (
            ...
            <div className="container my-3">
                <h1 className="text-center" style={{margin: '35px 0px'}}>MFW - Top Headlines</h1>
                {this.state.loading && <Spinner/>}
                <div className="row"> 
                {!this.state.loading && this.state.articles.map((element)=>{
                    return <div className="col-md-4" key={element.url}>
                        <NewsItem title={element.title?element.title:""} description={element.description?element.description.slice(0, 88):"Lorem ipsum dolor sit amet consectetur adipisicing elit. Similique pariatur accusan"} imageUrl={element.urlToImage} newsUrl={element.url} author={element.author} date={element.publishedAt} source={element.source.name}/>
                    </div> 
                })} 
                </div> 
